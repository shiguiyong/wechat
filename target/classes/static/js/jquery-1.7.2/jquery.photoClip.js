(function (root, factory) {
    ...
    function photoClip(container, option) {
        var size = option.size, outputSize = option.outputSize, file = option.file, view = option.view, ok = option.ok,
            outputType = "image/jpeg", loadStart = option.loadStart, loadComplete = option.loadComplete,
            loadError = option.loadError, clipFinish = option.clipFinish;
        if (!isArray(size)) {
            size = [260, 260];
        }
        ...
        var clipWidth = size[0] || 260, clipHeight = size[1] || 260, outputWidth = Math.max(outputSize[0], 0),
        ...
        var $container, $clipView, $moveLayer, $rotateLayer, $view, canvas, hammerManager, myScroll, containerWidth,
            containerHeight;
        init();
        initScroll();
        initEvent();
        ...
        function resetScroll() {
            ...
            curY = 0;
            curAngle = 0;
            $rotateLayer.css({"width": imgWidth, "height": imgHeight});
        ...
        function refreshScroll(width, height) {
            $moveLayer.css({"width": width, "height": height});
        ...
        function initEvent() {
            ...
            if (is_mobile) {
                ...
                $moveLayer.on("dblclick", function (e) {
                    rotateCW({x: e.clientX, y: e.clientY});
                });
            }
        }
        ...
        function rotateBy(angle, point) {
            ...
            var origin = calculateOrigin(curAngle, loacl), originX = origin.x, originY = origin.y, offsetX = 0,
                offsetY = 0, parentOffsetX = 0, parentOffsetY = 0, newAngle = curAngle + angle, curImgWidth,
                curImgHeight;
        ...
        function loaclToLoacl($layerOne, $layerTwo, x, y) {
            ...
            return {x: layerTwoOffset.left - layerOneOffset.left + x, y: layerTwoOffset.top - layerOneOffset.top + y};
        }
        ...
        function globalToLoacl($layer, x, y) {
            ...
            return {x: x + $win.scrollLeft() - layerOffset.left, y: y + $win.scrollTop() - layerOffset.top};
        }
        ...
        function init() {
            $container = $(container).css({"user-select": "none", "overflow": "hidden"});
    ...
    var prefix = '', transitionEnd;
    (function () {
        var eventPrefix, vendors = {Webkit: 'webkit', Moz: '', O: 'o'}, testEl = document.documentElement,
